...
commits.for.file <- structure(function(tfile){
  ...
  tfile <- "~/stringr/tests/testthat/test-detect.r"
  ...
  save(history, time.list, file="~/testthatQuantity/data/stringr.timings.RData")
  ...
  with.legend <- ggplot(to.show, aes(gmt.time, seconds, color=test.name))+
    geom_point(pch=1)+
    ggtitle("Times for Stringr tests, selected commits in 2014-2015")+
    xlab("Time of commit (GMT)")
  library(directlabels)
  direct.label(with.legend)
  ggsave("../stringr-detect-tests.png")
})
...
test.commit <- function(tfile, SHA1){
  ...
  test.df <- tryCatch({
    ...
    data.frame(test.name=paste(e), seconds=NA, status = "fail")
  })
...
### Saves the tests in tfile and runs them against all the commits specified
### by the SHA1.vec character vector
test.commits <- function(tfile, SHA1.vec) {
  stopifnot(is.character(tfile))
  stopifnot(length(tfile) == 1)
  ...
  save(history, version.list, file="~/testthatQuantity/data/stringr.timings2.RData")
  ver_times.df <- do.call(rbind, version.list)
  ver_times.df$gmt.time <- history[paste(ver_times.df$SHA1), "gmt.time"]
  rownames(ver_times.df) <- NULL
  last <- subset(ver_times.df, gmt.time > ISOdatetime(2015, 2, 15, 23, 30, 0, "GMT"))
  some <- subset(ver_times.df, gmt.time < ISOdatetime(2015, 2, 15, 0, 0, 0, "GMT"))
  to.show <- rbind(last, some)
  library(ggplot2)
  with.legend <- ggplot(to.show, aes(gmt.time, seconds, color=test.name))+
    geom_point(pch=1)+
    ggtitle("Times for current Stringr tests, against selected commits in 2014-2015")+
    xlab("Time of commit (GMT)")
  library(directlabels)
  direct.label(with.legend)
  ggsave("../stringr-detect-tests2.png")
}
