...
commits.for.file <- structure(function(tfile){
  ...
  save(history, time.list, file="~/TravisTests/data/stringr.timings.RData")
  ...
  last <- subset(time.df, gmt.time > ISOdatetime(2015, 1, 15, 23, 30, 0, "GMT"))
  some <- subset(time.df, gmt.time < ISOdatetime(2015, 1, 15, 0, 0, 0, "GMT"))
  ...
  ggsave("~/TravisTests/stringr-detect-tests.png")
})
...
test.commit <- function(tfile, SHA1){
  ...
  test.df <- tryCatch({
    ...
    ### Given the nature of the default reporter (StopReporter), the test 
    ### will come to a halt whenever an error takes place, thus allowing 
    ### the status to be update appropriately.
